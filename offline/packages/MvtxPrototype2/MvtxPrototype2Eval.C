...
int MvtxPrototype2Eval::Init(PHCompositeNode *topNode)
{
  ...
  h1d_nevents = new TH1D("h1d_nevents", ";Nevents", 1, -.5, .5);
	...
	{
	  h2d_hit[istave] = new TH2D(Form("h2d_hit_stave%d",istave),"", 9 * NCOL, -.5, (9*NCOL) - .5, NROW, -.5, NROW - .5);
    h2d_clu[istave] = new TH2D(Form("h2d_clu_stave%d",istave),"", 9 * NCOL, -.5, (9*NCOL) - .5, NROW, -.5, NROW - .5);
	}
