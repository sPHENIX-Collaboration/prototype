  ...
  if (p)
  {
    ...
    for (int iru=1; iru<NMAXRU+1; iru++)
    {
      ...
      for (int ich=1; ich<NMAXRUCHN+1; ich++)
      {
        ...
        if ( Verbosity()>100 )
          cout << "iru: " << iru << " ich: " << ich << " chip_id from packet: " << chip_id << " chip_id from map: " << chip_id_from_map << endl;
        ...
        for (int ihit=0; ihit<nhits_per_ch; ihit++){
          ...
          TrkrDefs::hitsetkey hitsetkey = MvtxDefs::genHitSetKey(lay, 0, chip_id_from_map);
