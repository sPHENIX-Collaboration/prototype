...
void RawTower_Prototype2::Reset()
{
  energy = 0;
  time = NAN;
  fill_n(signal_samples, NSAMPLES, -9999);
...
int RawTower_Prototype2::isValid() const { return get_energy() != 0; }

void RawTower_Prototype2::identify(std::ostream &os) const
{
  os << "RawTower_Prototype2: etabin: " << get_bineta()
     << ", phibin: " << get_binphi() << " energy=" << get_energy() << std::endl;
}

void RawTower_Prototype2::set_signal_samples(
    int i, RawTower_Prototype2::signal_type sig)
{
  assert(i >= 0);
  assert(i < NSAMPLES);
...
double RawTower_Prototype2::get_energy_power_law_exp(int verbosity)
{
  double peak = NAN;
  ...
  PROTOTYPE2_FEM::SampleFit_PowerLawExp(vec_signal_samples, peak, peak_sample,
                                        pedstal, verbosity);
