...
int PROTOTYPE3_FEM::GetHBDCh(const std::string &caloname, int i_column,
                             int i_row) {
  if (caloname == "HCALIN") {
    ...
    static int canmap[] = {
        ...
        //            Sean and John spot checked a number of these towers at
        //            BNL.  There could be mistakes, but cosmics look
        //            reasonable.
        //            Front view (looking downstream, same as above) but in HBD
        //            channel numbers
        ...
        3,  2,  19, 18, 35, 34, 51, 50, 1,  0,  17, 16, 33, 32, 49, 48, 7,
        6,  23, 22, 39, 38, 55, 54, 5,  4,  21, 20, 37, 36, 53, 52, 11, 10,
        27, 26, 43, 42, 59, 58, 9,  8,  25, 24, 41, 40, 57, 56, 15, 14, 31,
        30, 47, 46, 63, 62, 13, 12, 29, 28, 45, 44, 61, 60, 0};
...
bool PROTOTYPE3_FEM::SampleFit_PowerLawExp( //
  ...
  for (int ipoint = 0; ipoint < gpulse.GetN(); ipoint++)
    if ((gpulse.GetY())[ipoint] == 0 or
        (gpulse.GetY())[ipoint] >=
            4090) // drop point if touching max or low limit on ADCs
    {
  ...
  if (verbosity) {
    TCanvas *canvas = new TCanvas("PROTOTYPE3_FEM_SampleFit_PowerLawExp",
